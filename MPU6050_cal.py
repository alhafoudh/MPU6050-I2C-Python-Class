...
while True:
    ...
    if pidax.check_time():
        ...
        if axindex == len(x_accel_avg):
            axindex = 0
            print('x_accel_offset = %d' % avg_from_array(x_accel_offset_avg))
            print('y_accel_offset = %d' % avg_from_array(y_accel_offset_avg))
            print('z_accel_offset = %d' % avg_from_array(z_accel_offset_avg))
    ...
    if pidgx.check_time():
        ...
        if gxindex == len(x_gyro_avg):
            gxindex = 0
            # print('GYRO  x_avg_read: ' +
            #       str(avg_from_array(x_gyro_avg)) +
            #       ' x_avg_offset: ' +
            #       str(avg_from_array(x_gyro_offset_avg)))
            # print('GYRO  y_avg_read: ' +
            #       str(avg_from_array(y_gyro_avg)) +
            #       ' y_avg_offset: ' +
            #       str(avg_from_array(y_gyro_offset_avg)))
            # print('GYRO  z_avg_read: ' +
            #       str(avg_from_array(z_gyro_avg)) +
            #       ' z_avg_offset: ' +
            #       str(avg_from_array(z_gyro_offset_avg)))

            print('x_gyro_offset = %d' % avg_from_array(x_gyro_offset_avg))
            print('y_gyro_offset = %d' % avg_from_array(y_gyro_offset_avg))
            print('z_gyro_offset = %d' % avg_from_array(z_gyro_offset_avg))
            print('')
